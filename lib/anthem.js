...
// Önceden tamponlanmış yerel sesi anında çal. Promise döner (reddederse YouTube'a düşülür).
export function startLocalAnthem(fileOverride) {
  ...
  if (fileOverride && audioUrl !== fileOverride) { audioUrl = fileOverride; a.src = fileOverride; try { a.load(); } catch (_) {} }
  a.muted = false;
  a.volume = 0.95;
